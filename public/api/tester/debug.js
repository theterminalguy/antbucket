...
$(".fakeinputname").blur(function() {
  var newparamname = $(this).val().trim();
...
function postWithAjax(myajax) {
  myajax = myajax || {};
  myajax.url = host_url + $("#urlvalue").val().trim() + "/" + $("#optional_slug").val().trim();
...
function createUrlData(){
  var mydata = {};
	...
	for (i = 0; i < parameters.length; i++) {
		name = $(parameters).eq(i).attr("name").trim();
		if (name == undefined || name == "undefined") {
			continue;
		}
		value = $(parameters).eq(i).val().trim();
		mydata[name] = value
	}
  return(mydata);
}
